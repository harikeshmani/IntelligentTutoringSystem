...
<html>
	...
	<body>
					...
					<div class="row">
						<div class="col">
							<div class="heading heading-border heading-bottom-border">
								<h3>Payment <strong>Status</strong></h3>
							</div>
		...
		if (!$.trim(data)) {
		$('#Content').append('<tr><td><h4>No Record Found</h4></td></tr>');
		} else {
		$('#heads').append('<tr><th>Session With</th><th>Scheduled Date</th><th>payment Status</th></tr>');
		$.each(data, function(index, element) {
		$('#Content').append('<tr><td>' + element.name + '</td><td>' + element.session_date + '</td><td><button type="button" class="btn btn-primary" onclick=checkit(this) name ='+element.session_id+' value='+element.payment_status+'>'+element.payment_status+'</button></td></tr>');
		});
		}
		return false;
		});
